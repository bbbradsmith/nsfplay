...
@set PACONF=%PACONF% -DPA_BUILD_SHARED=OFF
@set PACONF=%PACONF% -DPA_BUILD_TESTS=OFF
@set PACONF=%PACONF% -DPA_BUILD_EXAMPLES=OFF
@set PACONF=%PACONF% -DPA_LIBNAME_ADD_SUFFIX=OFF
@set PACONF=%PACONF% -DPA_USE_WDMKS=OFF
REM hopefully restore WDMKS in the next version of PortAudio
@set PACONFD=%PACONF%
